...
public class ContentProposalAdapter {
  ...
  class ContentProposalPopup extends PopupDialog {
    ...
    private final class PopupCloserListener implements Listener {
      ...
      public void handleEvent(final Event e) {
        ...
        if (e.type == SWT.FocusOut) {
          ...
          /*
           * Ignore this event if it's only happening because focus is moving between the popup shells, their controls, or a scrollbar. Do this in an async
           * since the focus is not actually switched when this event is received.
           */
    ...
    private class InfoPopupDialog extends PopupDialog {
      ...
      protected void adjustBounds() {
        ...
        Rectangle rightProposedBounds =
            new Rectangle(parentBounds.x + parentBounds.width + PopupDialog.POPUP_HORIZONTALSPACING, parentBounds.y + PopupDialog.POPUP_VERTICALSPACING,
                parentBounds.width, parentBounds.height);
        rightProposedBounds = getConstrainedShellBounds(rightProposedBounds);
        // If it won't fit on the right, try the left
        if (rightProposedBounds.intersects(parentBounds)) {
          Rectangle leftProposedBounds =
              new Rectangle(parentBounds.x - parentBounds.width - POPUP_HORIZONTALSPACING - 1, parentBounds.y, parentBounds.width, parentBounds.height);
       ...
       */
      void setContents(String newContents_p) {
        String newContents = newContents_p;
        if (newContents_p == null) {
          newContents = EMPTY;
        }
     ...
     */
    ContentProposalPopup(String infoText, IContentProposal[] proposals_p) {
     ...
     */
    private void setProposals(IContentProposal[] newProposals_p) {
      IContentProposal[] newProposals = newProposals_p;
      if (newProposals_p == null || newProposals_p.length == 0) {
    ...
    /**
     * Opens this ContentProposalPopup. This method is extended in order to add the control listener when the popup is opened and to invoke the secondary popup
     * if applicable.
     ...
     */
    private void recomputeProposals(String filterText_p) {
      ...
      if (allProposals.length == 0) {
        ...
        setProposals(filterProposals(allProposals, filterText_p));
      }
    }

    /*
     * In an async block, request the proposals. This is used when clients are in the middle of processing an event that affects the widget content. By using an
     * async, we ensure that the widget content is up to date with the event.
     */
    private void asyncRecomputeProposals(final String filterText_p) {
      if (isValid()) {
        control.getDisplay().asyncExec(new Runnable() {
          public void run() {
            recordCursorPosition();
            recomputeProposals(filterText_p);
          }
        });
      } else {
        recomputeProposals(filterText_p);
    ...
    private IContentProposal[] filterProposals(IContentProposal[] proposals_p, String filterString) {
   ...
   * @param control_p the control for which the adapter is providing content assist. May not be <code>null</code>.
   * @param controlContentAdapter_p the <code>IControlContentAdapter</code> used to obtain and update the control's contents as proposals are accepted. May not
   *          be <code>null</code>.
   * @param proposalProvider_p the <code>IContentProposalProvider</code> used to obtain content proposals for this control, or <code>null</code> if no content
   *          proposal is available.
   * @param keyStroke the keystroke that will invoke the content proposal popup. If this value is <code>null</code>, then proposals will be activated
   *          automatically when any of the auto activation characters are typed.
   * @param autoActivationCharacters An array of characters that trigger auto-activation of content proposal. If specified, these characters will trigger
   *          auto-activation of the proposal popup, regardless of whether an explicit invocation keyStroke was specified. If this parameter is
   *          <code>null</code>, then only a specified keyStroke will invoke content proposal. If this parameter is <code>null</code> and the keyStroke
   *          parameter is <code>null</code>, then all alphanumeric characters will auto-activate content proposal.
   */
  public ContentProposalAdapter(Control control_p, IControlContentAdapter controlContentAdapter_p, IContentProposalProvider proposalProvider_p,
      KeyStroke keyStroke, char[] autoActivationCharacters) {
    super();
    // We always assume the control and content adapter are valid.
    Assert.isNotNull(control_p);
    Assert.isNotNull(controlContentAdapter_p);
    this.control = control_p;
    this.controlContentAdapter = controlContentAdapter_p;

    // The rest of these may be null
    this.proposalProvider = proposalProvider_p;
    this.triggerKeyStroke = keyStroke;
    if (autoActivationCharacters != null) {
      this.autoActivateString = new String(autoActivationCharacters);
    }
    addControlListener(control_p);
  }
   ...
   * @param labelProvider_p the (@link ILabelProvider} used to show proposals.
   */
  public void setLabelProvider(ILabelProvider labelProvider_p) {
    this.labelProvider = labelProvider_p;
  }

  /**
   * Return the proposal provider that provides content proposals given the current content of the field. A value of <code>null</code> indicates that there are
   * no content proposals available for the field.
   ...
   * @param proposalProvider_p the {@link IContentProposalProvider} used to show proposals
   */
  public void setContentProposalProvider(IContentProposalProvider proposalProvider_p) {
    this.proposalProvider = proposalProvider_p;
  }
   ...
   *         proposal popup, regardless of whether an explicit invocation keyStroke was specified. If this parameter is <code>null</code>, then only a specified
   *         keyStroke will invoke content proposal. If this value is <code>null</code> and the keyStroke value is <code>null</code>, then all alphanumeric
   ...
   * @param autoActivationCharacters An array of characters that trigger auto-activation of content proposal. If specified, these characters will trigger
   *          auto-activation of the proposal popup, regardless of whether an explicit invocation keyStroke was specified. If this parameter is
   *          <code>null</code>, then only a specified keyStroke will invoke content proposal. If this parameter is <code>null</code> and the keyStroke value is
   ...
   * @param acceptance a constant indicating how an accepted proposal should affect the control's content. Should be one of <code>PROPOSAL_INSERT</code>,
   ...
   * narrowing and navigating the list of proposals provided once the popup is open. Filtering of the proposals will occur even when the control content is not
   * affected by user typing. Note that automatic filtering is not used to achieve content-sensitive filtering such as auto-completion. Filtering that is
   * sensitive to changes in the control content should be performed by the supplied {@link IContentProposalProvider}.
   * @param filterStyle_p a constant indicating how keystrokes received in the proposal popup affect filtering of the proposals shown. <code>FILTER_NONE</code>
   ...
   *          specifies that the content of the popup will be filtered by the most recently typed character. <code>FILTER_CUMULATIVE</code> is deprecated and no
   *          longer recommended. It specifies that the content of the popup will be filtered by a string containing all the characters typed since the popup
   *          has been open.
   */
  public void setFilterStyle(int filterStyle_p) {
    this.filterStyle = filterStyle_p;
  }
   ...
   * to the adapted control when the proposal popup is open.
   * @param propagateKeys_p a boolean that indicates whether key events (including auto-activation characters) should be propagated to the adapted control when
   *          the proposal popup is open.
   */
  public void setPropagateKeys(boolean propagateKeys_p) {
    this.propagateKeys = propagateKeys_p;
  }
   ...
   * @param enabled <code>true</code> if the adapter is enabled and responding to user input, <code>false</code> if it is ignoring user input.
   */
  ...
  /**
   * Add the specified listener to the list of content proposal listeners that are notified when content proposals are chosen. </p>
   * @param listener the IContentProposalListener to be added as a listener. Must not be <code>null</code>. If an attempt is made to register an instance which
   *          is already registered with this instance, this method has no effect.
   * @see IContentProposalListener3
   */
  ...
  /**
   * Removes the specified listener from the list of content proposal listeners that are notified when content proposals are chosen. </p>
   * @param listener the IContentProposalListener to be removed as a listener. Must not be <code>null</code>. If the listener has not already been registered,
   *          this method has no effect.
   * @since 3.3
   * @see IContentProposalListener3
   */
  public void removeContentProposalListener(IContentProposalListener3 listener) {
   ...
   */
  private void addControlListener(Control control_p) {
    if (DEBUG) {
    ...
    control_p.addListener(SWT.KeyDown, controlListener);
    control_p.addListener(SWT.Traverse, controlListener);
    control_p.addListener(SWT.Modify, controlListener);
   ...
   * @param autoActivated a boolean indicating whether the popup was autoactivated. If false, a beep will sound when no proposals can be shown.
   */
  protected void openProposalPopup(boolean autoActivated) {
    if (isValid()) {
      if (popup == null) {
   ...
   * proposal to be selected. This method is used by subclasses to explicitly invoke the opening of the popup. If there are no proposals to show, the popup will
   * not open and a beep will be sounded.
   */
  ...
  private void notifyProposalAccepted(IContentProposal proposal) {
    ...
    for (int i = 0; i < listenerArray.length; i++) {
      ((IContentProposalListener3) listenerArray[i]).proposalAccepted(proposal);
    }
  }
  ...
  /*
   * Return whether a proposal popup should remain open. If it was autoactivated by specific characters, and none of those characters remain, then it should not
   * remain open. This method should not be used to determine whether autoactivation has occurred or should occur, only whether the circumstances would dictate
   * that a popup remain open.
   */
