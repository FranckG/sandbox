...
import {0}.application.{1}Application;
...
public class {1}RemoteConnector extends AbstractConnector implements IRemoteConnector '{
  ...
  public CommandStatus closeApplication() throws Exception {
    // To allow this ECF call to complete, call stop asynchronously.
    new Thread(new Runnable() {
      public void run() {
        try {
          // Wait for this ECF call to complete.
          Thread.sleep(1000);
        } catch (InterruptedException exception_p) {
          exception_p.printStackTrace();
        }
        '{1}Application'.__instance.stop();
      }
    }).start();
