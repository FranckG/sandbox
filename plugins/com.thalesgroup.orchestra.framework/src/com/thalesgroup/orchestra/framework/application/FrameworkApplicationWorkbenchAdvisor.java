...
import com.thalesgroup.orchestra.framework.server.FolderDeletionJob;
import com.thalesgroup.orchestra.framework.server.ServerManager;

public class FrameworkApplicationWorkbenchAdvisor extends WorkbenchAdvisor {
  /**
   * Contexts loaded from the admin contexts pool.
   */
  private List<RootContextsProject> _contextsInPool;
  /**
   ...
   * @param manager_p
   */
  public FrameworkApplicationWorkbenchAdvisor(ServerManager manager_p, IStatus initialContextErrors_p, List<RootContextsProject> contextsInPool_p) {
    _manager = manager_p;
  ...
  public WorkbenchWindowAdvisor createWorkbenchWindowAdvisor(IWorkbenchWindowConfigurer configurer) {
    FrameworkWorkbenchWindowAdvisor frameworkWorkbenchWindowAdvisor =
        new FrameworkWorkbenchWindowAdvisor(configurer, _manager, _initialContextErrors, _contextsInPool);
    _initialContextErrors = null;
    return frameworkWorkbenchWindowAdvisor;
  }
   ...
   * @param shared <code>true</code> if this is a shared image, and <code>false</code> if this is not a shared image
  ...
  public String getInitialWindowPerspectiveId() {
    return FrameworkPerspective.class.getName();
  }
  ...
  public boolean preShutdown() {
    // Search for folder deletion jobs that should be terminated before exiting.
    Job[] jobs = Job.getJobManager().find(FolderDeletionJob.JOB_FAMILY_FOLDER_DELETION);
    // Precondition.
    ...
    DisplayHelper.displayRunnable(new AbstractRunnableWithDisplay() {
      ...
      public void run() {
        forceExit[0] =
            MessageDialog.openConfirm(getDisplay().getActiveShell(), Messages.FrameworkApplicationWorkbenchAdvisor_ExitConfirmationDialog_Title,
                Messages.FrameworkApplicationWorkbenchAdvisor_ExitConfirmationDialog_Message);
      }
    }, false);
    return forceExit[0];
  }
